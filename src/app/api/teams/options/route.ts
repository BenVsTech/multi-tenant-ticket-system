...
import { getTeamsForOptions } from "@/lib/service/team.service";
...
export async function GET(request: NextRequest): Promise<NextResponse<DataReturnObject<{id: number, name: string}[]>>> {
    return apiHandler<{id: number, name: string}[]>(async () => {
        ...
        const getTeamsForOptionsResult = await getTeamsForOptions(userId, accountId);
        if(!getTeamsForOptionsResult.status || !getTeamsForOptionsResult.data) {
            return {
                status: false,
                data: null,
                message: getTeamsForOptionsResult.message
            };
        }

        return {
            status: true,
            data: getTeamsForOptionsResult.data,
            message: getTeamsForOptionsResult.message
        };

    }, 'GET /api/teams/options', 200, 400);
}
